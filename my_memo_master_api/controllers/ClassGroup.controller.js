...
exports.create = async (req, res) => {
  try {
    ...
    if (result === false) return res.status(403).json({ message: "Accès refusé. Seuls les administrateurs peuvent créer des groupes." });
...
exports.update = async (req, res) => {
  try {
    ...
    if (result === false) return res.status(403).json({ message: "Accès refusé. Seuls les administrateurs peuvent modifier des groupes." });
...
exports.delete = async (req, res) => {
  try {
    ...
    if (result === false) return res.status(403).json({ message: "Accès refusé. Seuls les administrateurs peuvent supprimer des groupes." });
    if (result === null) return res.status(404).json({ message: "Groupe introuvable." });
...
exports.addMember = async (req, res) => {
  try {
    ...
    if (result === false) return res.status(403).json({ message: "Accès refusé. Seuls les administrateurs peuvent gérer les membres." });
    if (result === null) return res.status(404).json({ message: "Groupe introuvable." });
...
exports.removeMember = async (req, res) => {
  try {
    const result = await ClassGroupService.removeMember(req.params.id, req.params.userId, req.user.id);
    if (result === false) return res.status(403).json({ message: "Accès refusé. Seuls les administrateurs peuvent gérer les membres." });
    if (result === null) return res.status(404).json({ message: "Membre introuvable dans ce groupe." });
