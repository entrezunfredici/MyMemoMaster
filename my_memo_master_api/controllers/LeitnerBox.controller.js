...
exports.findOne = async (req, res) => {
  try {
    ...
    if (!box) {
      res.status(404).send({ message: `Boîte introuvable pour l'ID ${req.params.id}.` })
    } else {
      res.status(200).send(box)
    }
  } catch (error) {
    logger.error(error?.message || error)
    res.status(500).send({ message: 'Erreur lors de la récupération de la boîte: ' })
  }
}
...
exports.create = async (req, res) => {
  try {
    ...
    logger.error(error?.message || error)
    res.status(500).send({ message: 'Erreur lors de la création de la boîte.' })
  }
}
...
exports.update = async (req, res) => {
  try {
    ...
    if (!updatedBox) {
      return res.status(404).json({ message: `Boîte introuvable pour l'ID ${id}.` })
    }
...
exports.delete = async (req, res) => {
  try {
    ...
    logger.error(error?.message || error)
    res.status(500).send({ message: 'Erreur lors de la suppression de la boîte.' })
  }
}
