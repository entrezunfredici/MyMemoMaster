...
exports.create = async (req, res) => {
  try {
    ...
    if (result === false) return res.status(403).json({ message: "Accès refusé. Vous devez être enseignant dans le groupe associé à cette séance." });
...
exports.update = async (req, res) => {
  try {
    ...
    if (result === false) return res.status(403).json({ message: "Accès refusé. Seul le créateur peut modifier cette échéance." });
...
exports.delete = async (req, res) => {
  try {
    ...
    if (result === null) return res.status(404).json({ message: "Échéance introuvable." });
    if (result === false) return res.status(403).json({ message: "Accès refusé. Seul le créateur peut supprimer cette échéance." });
