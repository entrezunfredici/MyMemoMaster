...
exports.create = async (req, res) => {
  try {
    ...
    if (result === false)
      return res
        .status(403)
        .json({ message: 'Accès refusé. Seuls les administrateurs peuvent créer des événements.' })
  ...
  try {
    ...
    if (result === false)
      return res
        .status(403)
        .json({
          message: 'Accès refusé. Seuls les administrateurs peuvent modifier des événements.'
        })
  ...
  try {
    ...
    if (result === false)
      return res
        .status(403)
        .json({
          message: 'Accès refusé. Seuls les administrateurs peuvent supprimer des événements.'
        })
    if (result === null) return res.status(404).json({ message: 'Événement introuvable.' })
...
exports.addOccurrence = async (req, res) => {
  try {
    ...
    const result = await CalendarEventService.addOccurrence(req.params.id, req.user.id, {
      date,
      startTime,
      endTime
    })
  ...
  try {
    ...
    if (error?.name === 'SequelizeForeignKeyConstraintError') {
      return res
        .status(409)
        .json({
          message: 'Impossible de supprimer cette occurrence : des échéances y sont rattachées.'
        })
    }
    logger.error(error?.message || error)
