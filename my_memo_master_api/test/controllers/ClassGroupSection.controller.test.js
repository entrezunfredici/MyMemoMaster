...
jest.mock('../../services/ClassGroupSection.service', () => ({
  findAll: jest.fn(),
  create: jest.fn(),
  update: jest.fn(),
  delete: jest.fn()
...
const service = require('../../services/ClassGroupSection.service')

const BASE = '/api/v1'
const makeToken = (payload = { id: 1 }) => jwt.sign(payload, 'test-secret', { expiresIn: '1d' })

const mockSection = { id: 1, title: 'Chapitre 1', type: 'section', classGroupId: 1, createdBy: 1 }

describe('ClassGroupSection Controller', () => {
  beforeEach(() => jest.clearAllMocks())

  // ── GET /class-groups/:id/sections ───────────────────────────────────────────

  describe('GET /class-groups/:id/sections', () => {
    it('200 — retourne les sections du groupe', async () => {
      service.findAll.mockResolvedValue([mockSection])

      const res = await request(app)
        .get(`${BASE}/class-groups/1/sections`)
    ...
    it('401 — pas de token', async () => {
      const res = await request(app).get(`${BASE}/class-groups/1/sections`)
  ...
  // ── POST /class-groups/:id/sections ──────────────────────────────────────────

  describe('POST /class-groups/:id/sections', () => {
    const validBody = { title: 'Chapitre 1', type: 'section' }

    it('201 — section créée', async () => {
      service.create.mockResolvedValue(mockSection)

      const res = await request(app)
        .post(`${BASE}/class-groups/1/sections`)
        .set('Authorization', `Bearer ${makeToken()}`)
        .send(validBody)
    ...
    it('400 — type invalide', async () => {
      const res = await request(app)
        .post(`${BASE}/class-groups/1/sections`)
        .set('Authorization', `Bearer ${makeToken()}`)
        .send({ title: 'Chapitre 1', type: 'invalide' })
  ...
  // ── PUT /class-groups/:id/sections/:sectionId ─────────────────────────────────

  describe('PUT /class-groups/:id/sections/:sectionId', () => {
    it('200 — section mise à jour', async () => {
      service.update.mockResolvedValue({ ...mockSection, title: 'Nouveau titre' })

      const res = await request(app)
        .put(`${BASE}/class-groups/1/sections/1`)
        .set('Authorization', `Bearer ${makeToken()}`)
    ...
    it('404 — section introuvable', async () => {
      service.update.mockResolvedValue(null)

      const res = await request(app)
        .put(`${BASE}/class-groups/1/sections/99`)
        .set('Authorization', `Bearer ${makeToken()}`)
        .send({ title: 'Nouveau titre' })
  ...
  // ── DELETE /class-groups/:id/sections/:sectionId ──────────────────────────────

  describe('DELETE /class-groups/:id/sections/:sectionId', () => {
    it('200 — section supprimée', async () => {
      service.delete.mockResolvedValue(true)

      const res = await request(app)
        .delete(`${BASE}/class-groups/1/sections/1`)
        .set('Authorization', `Bearer ${makeToken()}`)
    ...
    it('401 — pas de token', async () => {
      const res = await request(app).delete(`${BASE}/class-groups/1/sections/1`)
      expect(res.status).toBe(401)
    })
    ...
    it('404 — section introuvable', async () => {
      ...
      const res = await request(app)
        .delete(`${BASE}/class-groups/1/sections/99`)
        .set('Authorization', `Bearer ${makeToken()}`)
