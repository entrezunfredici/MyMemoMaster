...
describe('FieldsType Controller', () => {
  ...
  describe('POST /fieldstypes', () => {
    ...
    it('201 — crée un type de champ', async () => {
      ...
      const res = await request(app).post(`${BASE}/fieldstypes`).send(validBody)
    ...
    it('400 — name manquant', async () => {
      const res = await request(app).post(`${BASE}/fieldstypes`).send({ allowUnit: true })
    ...
    it('400 — allowUnit manquant', async () => {
      const res = await request(app).post(`${BASE}/fieldstypes`).send({ name: 'Nombre' })
  ...
  describe('PUT /fieldstypes/:id', () => {
    it('200 — met à jour le type de champ', async () => {
      ...
      const res = await request(app).put(`${BASE}/fieldstypes/1`).send({ name: 'Texte long' })
    ...
    it('404 — type introuvable', async () => {
      ...
      const res = await request(app).put(`${BASE}/fieldstypes/99`).send({ name: 'Texte long' })

      expect(res.status).toBe(404)
    })

    it('400 — name trop court (si fourni)', async () => {
      const res = await request(app).put(`${BASE}/fieldstypes/1`).send({ name: 'X' })

      expect(res.status).toBe(400)
    })

    it("400 — allowUnit n'est pas un booléen (si fourni)", async () => {
      const res = await request(app).put(`${BASE}/fieldstypes/1`).send({ allowUnit: 'maybe' })
