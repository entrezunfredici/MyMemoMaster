...
describe('Grading Controller — validation', () => {
  ...
  describe('POST /grading/date', () => {
    it('200 — retourne le résultat de correction', async () => {
      gradingService.gradeDateAnswer.mockReturnValue({ is_correct: true, score: 1.0, strategy: 'date', explanation: 'ok' });
  ...
  describe('POST /grading/semantic', () => {
    it('200 — correct_answers est une string', async () => {
      semanticService.gradeSemantic.mockResolvedValue({ is_correct: true, score: 0.9, strategy: 'semantic', explanation: 'ok' });

      const res = await request(app)
        .post(`${BASE}/grading/semantic`)
        .send({ correct_answers: 'La photosynthèse produit du glucose.', student_answer: 'Les plantes produisent du glucose.' });
    ...
    it('200 — correct_answers est un tableau de strings', async () => {
      semanticService.gradeSemantic.mockResolvedValue({ is_correct: false, score: 0.3, strategy: 'semantic', explanation: 'non' });
