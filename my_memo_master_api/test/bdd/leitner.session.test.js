...
const { syncModels, LeitnerSystem, LeitnerBox, LeitnerCard, Question, Response, User, Role } = require('../../models');
...
describe('Leitner — session complète (tests fonctionnels)', () => {
  ...
  it('Session multi-cartes — plusieurs cartes dues retournées ensemble', async () => {
    ...
    const q2 = await Question.create({ statement: 'Capitale de l\'Espagne ?', questionPosition: 2, type: 'text' });
  ...
  it('POST /response — carte inexistante — 404', async () => {
    semanticService.gradeSemantic.mockResolvedValueOnce({ is_correct: true, score: 1, explanation: '', decision_zone: 'high' });
