...
module.exports = {
  async up(queryInterface) {
    ...
    await queryInterface.addIndex("CalendarEvent", ["classGroupId"], { name: "idx_calendarevent_classgroupid" });
    await queryInterface.addIndex("CalendarEvent", ["createdBy"], { name: "idx_calendarevent_createdby" });
    await queryInterface.addIndex("EventOccurrence", ["eventId"], { name: "idx_eventoccurrence_eventid" });
    await queryInterface.addIndex("EventOccurrence", ["date"], { name: "idx_eventoccurrence_date" });
    await queryInterface.addIndex("Deadline", ["occurrenceId"], { name: "idx_deadline_occurrenceid" });
    ...
    await queryInterface.addIndex("RevisionSession", ["userId"], { name: "idx_revisionsession_userid" });
