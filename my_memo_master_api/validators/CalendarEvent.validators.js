...
exports.create = [
  ...
  body('classGroupId')
    .notEmpty()
    .withMessage('Le groupe classe est requis.')
    .isInt({ min: 1 })
    .withMessage('classGroupId doit être un entier positif.'),
  body('recurrenceMode')
    .notEmpty()
    .withMessage('Le mode de récurrence est requis.')
    .isIn(['manual', 'auto'])
    .withMessage("recurrenceMode doit être 'manual' ou 'auto'."),
  body('recurrenceRule')
    .if(body('recurrenceMode').equals('auto'))
    .notEmpty()
    .withMessage('La règle de récurrence est requise en mode auto.')
    .isObject()
    .withMessage('recurrenceRule doit être un objet.')
    .custom((rule) => {
  ...
  body('occurrences')
    ...
    .optional()
    .isArray()
    .withMessage('occurrences doit être un tableau.'),
  body('occurrences.*.date')
    .optional()
    .isDate({ format: 'YYYY-MM-DD' })
    .withMessage('Chaque occurrence doit avoir une date valide.'),
  body('occurrences.*.startTime')
    .optional()
    .matches(timeRegex)
    .withMessage('startTime invalide dans les occurrences.'),
  body('occurrences.*.endTime')
    .optional()
    .matches(timeRegex)
    .withMessage('endTime invalide dans les occurrences.')
]
...
exports.addOccurrence = [
  ...
  body('startTime')
    ...
    .matches(timeRegex)
    .withMessage('Format HH:MM requis.'),
  body('endTime')
    .notEmpty()
