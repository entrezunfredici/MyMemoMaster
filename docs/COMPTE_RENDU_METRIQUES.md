...
**Date d'arrêté des mesures** : 2026-09-01
...
|---|---|---|---|---|
| Avancement | Tâches validées / total | % | **87,5 %** à l'étape « validé » sur le périmètre engagé (217/248) — **repris du 2026-08-31** ; 67,8 % rapporté aux 320 sous-tâches | ⚠️ **Non recompté à cet arrêté** (voir encadré en tête) — dernière recomputation octet pour octet au 2026-08-31 ; non contredit par le total brut (350 tâches) et le graphe de dépendances revérifiés identiques à cet arrêté |
| Coûts | JH consommés × 300 €/j | € | **86 250 €** validés (287,5 JH) ; enveloppe planifiée **108 675 €** (362,3 JH) ; reste à faire **22 425 €** | ⚠️ Charge planifiée, aucune saisie de temps possible — **repris du 2026-08-31, non recalculé à cet arrêté** |
| Délais | Dates réelles vs Gantt prévisionnel | jours d'écart | **30 tâches** au-delà de leur échéance (598 h, 74,8 JH), dont 20 sur `C-01`/`C-02` ; plan clos au 21/07/2026, dépassé de **41 jours** | ⚠️ Le plan est condensé — voir §4. **Repris du 2026-08-31, non recalculé à cet arrêté** |
| Risques | Dépendances bloquantes non levées | nb | **183 liens** ouverts, **78 verrous** distincts, **91 tâches** immobilisées — plus **7 dépendances d'infrastructure** (+1 à cet arrêté : gel de la CD, `K8S_*_ENABLED`, §5.2 point 7) | ✅ Mesure directe, revérifiée à cet arrêté — inchangée sur Odoo, +1 point d'exploitation déjà connu mais non encore inventorié ici |
| RH | Charge par profil / capacité disponible | JH | **362 JH pour 483 JH de capacité ≈ 75 %**, réparti sur **7 profils** ; pointe à 113 % (SysAdmin) | ⚠️ **Repris du 2026-08-31, non recalculé à cet arrêté** ; répartition par profil non rejouée depuis le 2026-08-28 (planning d'équipe inchangé) |
| Qualité | Couverture de tests SonarQube | % | **63,9 %** mesurée par l'instance auto-hébergée (dernière analyse effectivement lue, 2026-08-29) ; reproduite en local à cet arrêté à **81,49 % API / 61,31 % front** | ⚠️ 7 analyses Sonar de plus depuis (vertes en CI) mais non relues en détail, accès anonyme testé et refusé (401), pas de jeton local (§7.1) |
...
| **P2** | Versionner le script de filtrage « périmètre engagé » (217/248, exclusion synthèse + backlog) — refait de mémoire à chaque arrêté depuis 2026-08-28, absent du dépôt | Méthode (fiabilité des arrêtés) | ~0,5 JH |
...
| Avancement, coûts, délais, RH (217/248, 2 898 h, 75 %) | **Non rejoué à cet arrêté** — chiffres repris du 2026-08-31 sans recomputation de la ventilation « périmètre engagé » (script de filtrage absent du dépôt, voir action P2 §8) ; seule l'extraction brute ci-dessus a été revérifiée |
| État `state` de `QA.03`/`QA.05`/`QA.06` | Filtré depuis la même extraction Odoo (`odoo_cli.py read project.task --domain '[["project_id","=",15],["name","like","QA.0"]]' --fields id,name,stage_id,state`) — rejoué à cet arrêté : toujours `01_in_progress` malgré l'étape « validé » |
...
| État d'exécution CI d'un commit | `GET /repos/entrezunfredici/MyMemoMaster/commits/<sha>/check-runs` (API GitHub publique, sans jeton) — rejoué à cet arrêté sur `8baf9f6`, `18b843f`, `c874bc7` (le SHA court `3589053` seul ne renvoie aucun check-run : poussé dans le même lot que `18b843f`, GitHub n'attache les checks qu'à la tête de push) : tous verts sur `test_and_lint` (API+front), `SonarQube Analysis`, `Parcours E2E + charge`, `Build and Push Docker Images` ; `Deploy to Kubernetes (prod)`/`(preprod)` en `skipped` sur les trois |
...
| Couverture API | `cd my_memo_master_api && npx jest --coverage` — rejoué à cet arrêté : **1 560/1 560**, 81,49 % |
| Tests front + couverture | `cd my_memo_master_front && npx vitest run --coverage` — rejoué à cet arrêté : **708/708**, 61,31 % |
