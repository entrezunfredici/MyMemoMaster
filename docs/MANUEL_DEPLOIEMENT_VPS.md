...
> Branche git : `dev` · Fichier de déploiement : [server_docker_compose/docker-compose.yml](../server_docker_compose/docker-compose.yml) · Déploiement automatisé par [.github/workflows/cd.yml](../.github/workflows/cd.yml) (job `deploy_test`).
...
| `api` | `fredissimo/mymemomaster_test_api:latest` | API Express | `https://${API_DOMAIN}` via Traefik |
| `front` | `fredissimo/mymemomaster_test_front:latest` | Front Vue 3 servi par nginx | `https://${FRONT_DOMAIN}` via Traefik |
| `pgadmin` | pgadmin | Administration BDD | `https://${PGADMIN_DOMAIN}` via Traefik |
...
# Copier server_docker_compose/.env.example du dépôt vers .env, puis remplir :
nano .env
```

Le template commenté est [server_docker_compose/.env.example](../server_docker_compose/.env.example). Règles :
...
docker compose --env-file .env up -d pgadmin api front backup
...
1. **Valide la syntaxe** du docker-compose (avec le `.env.example`) avant de toucher au serveur ;
...
6. Tire les nouvelles images (`pull`), redémarre (`up -d`), puis **boucle de vérification santé** : jusqu'à 24 tentatives espacées de 5 s sur l'état `healthy` de postgres/api/front — en cas d'échec, le job dump `compose ps` + les 100 dernières lignes de logs et sort en erreur ;
...
| Logs applicatifs | `docker compose --env-file .env logs -f api` |
