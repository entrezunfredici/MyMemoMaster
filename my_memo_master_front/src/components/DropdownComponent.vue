<template>
  <Menu as="div" class="relative inline-block text-left py-2 w-full">
    <div>
      <button
        class="flex justify-between w-full items-center justify-center bg-white border-2 border-gray rounded-lg px-2 py-1 text-lg font-medium text-dark"
    ...
    <TransitionRoot :show="show">
      <TransitionChild v-click-outside="() => show = false" as="div" enter="transition ease-out duration-100 transform"
        ...
        class="origin-top-right absolute mt-2 w-full shadow-xl rounded-lg bg-white p-4 z-10">
        <slot></slot>
      </TransitionChild>
    </TransitionRoot>
  </Menu>
