<template>
  <div class="space-y-6 px-2 sm:px-4 lg:px-6">
    <h1 class="text-2xl font-bold text-dark">Administration plateforme</h1>
        ...
        <table class="w-full text-sm">
          ...
          <thead>
            <tr class="border-b border-gray text-left text-xs text-dark/60 uppercase tracking-wide">
              ...
              <th scope="col" class="pb-2 pr-4">Admin</th>
    ...
    <section v-if="selectedId" class="rounded-2xl border-2 border-gray bg-white shadow-sm">
      ...
      <!-- Sous-onglets -->
      <div class="flex gap-1 border-b border-gray px-4 pt-3">
        <button v-for="tab in detailTabs" :key="tab.key"
          @click="activateDetailTab(tab.key)"
          :class="[detailTab === tab.key
            ? 'border-b-2 border-primary text-primary font-semibold'
            : 'text-dark/60 hover:text-dark', 'pb-2 px-2 text-sm transition']">
          {{ tab.label }}
        </button>
      </div>
      ...
      <div class="p-4">
        ...
        <div v-if="detailTab === 'gerant'" class="space-y-4">
          ...
          <div class="rounded-xl border border-primary/30 bg-primary/5 p-4 space-y-3">
            <p class="text-sm font-semibold text-dark">Assigner un gérant</p>
            ...
            <div class="flex gap-2">
              <button @click="submitAssignAdmin"
                :disabled="gerantForm.loading"
...
const gerantForm = reactive({ email: '', loading: false, error: '' })

const createForm = reactive({ name: '', code: '', adminId: null, error: '' })
const editForm = reactive({ visible: false, id: null, name: '', code: '', adminId: null, error: '' })
...
async function submitCreate() {
  ...
  const created = await etabStore.createEtab({
    ...
    adminId: createForm.adminId ? Number(createForm.adminId) : null,
  })
  if (created) {
    showCreate.value = false
    Object.assign(createForm, { name: '', code: '', adminId: null, error: '' })
  }
}

function openEdit(etab) {
  Object.assign(editForm, {
    visible: true, id: etab.id,
    name: etab.name, code: etab.code,
    adminId: etab.adminId ?? null, error: ''
  })
}
...
async function submitEdit() {
  ...
  const ok = await etabStore.updateEtab(editForm.id, {
    ...
    adminId: editForm.adminId ? Number(editForm.adminId) : null,
  })
...
async function submitAssignAdmin() {
  ...
  if (result) {
    ...
    gerantForm.error = "Aucun compte trouvé pour cet email ou erreur serveur."
  }
}
