  ...
  <div class="grid grid-cols-1 grid-rows-2 lg:grid-cols-2 lg:grid-rows-1 gap-1.5 bg-primary rounded-[10px] p-1.5">
    <section class="order-2 lg:order-1 p-5 bg-light rounded-[7px]">
      <div class="w-full">
        ...
        <div class="py-4">
          <span class="text-lg font-medium text-light-primary">Question</span>
        ...
        <div class="py-4">
          <span class="text-lg font-medium text-light-primary">Réponse</span>
          <textarea v-model="response.content" placeholder="Quelle est la réponse ?" class="w-full p-2 rounded-lg text-dark" />
        </div>

        <div class="py-4 flex justify-end">
          <button class="flex items-center py-3 gap-2 bg-primary rounded-lg px-5 text-white hover:bg-primary-hover transition">
            Insert image
          </button>
        </div>
        ...
        <div class="border-4 border-primary rounded-lg px-8 py-4 mt-6">
          ...
          <div class="flex flex-col gap-4">
            <div v-for="(q) in questions" :key="q.idQuestion"
              class="flex justify-between border-2 bg-white border-gray rounded-lg px-4 py-2">
