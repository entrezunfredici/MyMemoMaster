  ...
  setup() {
    ...
    const filteredDiagrams = computed(() => {
      return diagrams.value.filter((diagram) => {
        ...
        const matchSubject = selectedSubject.value
          ? String(diagram.subjectId) === String(selectedSubject.value)
          : true
    ...
    const toggleEditMode = () => {
      ...
      toast[editMode.value ? 'success' : 'warning'](
        `Mode edition ${editMode.value ? 'active' : 'desactive'}`
      )
    }

    const toggleDeleteMode = () => {
      deleteMode.value = !deleteMode.value
      toast[deleteMode.value ? 'success' : 'warning'](
        `Mode suppression ${deleteMode.value ? 'active' : 'desactive'}`
      )
    }
    ...
    const loadDiagram = (diagram) => {
      ...
      try {
        const jsonData =
          typeof diagram.mindMapJson === 'string' ? JSON.parse(diagram.mindMapJson) : diagram.mindMapJson
