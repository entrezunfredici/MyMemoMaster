...
- **Certificat `mmm-preprod-pgadmin-tls` non émis.** Le challenge DNS-01 est pourtant `valid` (« Successfully authorized domain ») ; c'est la finalisation ACME qui échoue sur `404 urn:ietf:params:acme:error:malformed: Certificate not found`, une erreur transitoire côté Let's Encrypt. cert-manager retente en backoff exponentiel. Les certificats `api` et `front` ont été émis normalement. Sans impact sur l'application — seul pgAdmin est concerné.
