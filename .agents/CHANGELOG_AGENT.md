...
| Audit RGAA 4.1.2 — les 106 critères | **EN COURS** (ouvert 2026-08-30, poursuivi 2026-08-31 — **57 % posé, 60/106 verdicts**) — `docs/AUDIT_RGAA_106.md`. Thématiques **1 (Images), 5 (Tableaux), 6 (Liens) closes, 0 non-conformité restante** ; progrès partiels sur 4, 8, 9, 11, 12, 13 ; 3, 7, 10 encore ouvertes. **9 non-conformités trouvées puis corrigées le 2026-08-31** : **6.2 sévère** — `aria-label` ajouté sur les ~11 liens de la nav principale (`App.vue`, desktop + mobile), test de régression `test/App.test.js` ; 5.4-5.7 — `<caption>`/`scope="col"` ajoutés aux 4 tableaux de données ; 11.6/11.7 — `role="radiogroup"`/`"group"` + `aria-labelledby` sur les 5 regroupements (QCM radio, sélection de jours) ; 11.13 — `autocomplete` ajouté aux formulaires d'authentification ; 1.2 — `alt=""` par défaut sur l'illustration décorative. **3 candidats restent ouverts** (9.2 saut de titre, 8.6 titres en anglais — question éditoriale, 12.7 lien d'évitement), non corrigés faute d'être dans le lot proposé ou d'avoir été confirmés visuellement. Vérifié : 705/705 tests (+2), 0/79 audit statique, ESLint propre | 2026-08-31 |
...
| Cluster Kubernetes (Infomaniak PCK) | Provisionné — cluster `pck-dkoyol2`, **3 workers** `a4-ram8-disk20-perf1` (4 vCPU / 8 Go ; allocatable réel : 3,92 vCPU / 5,5 Gi par nœud, soit 11,76 vCPU / 16,5 Gi au total), K8s v1.36.3, CNI Cilium, CSI Cinder + OpenStack CCM. Le 3ᵉ nœud (`…-5sx65`) a été ajouté le **2026-08-27 08:33 UTC**, après l'arbitrage à 2 nœuds du 2026-08-26 — constaté le 2026-08-28 via `kubectl get nodes`. Addons installés le 2026-08-26 : ingress-nginx (2 replicas, LB Octavia `83.228.249.190`, real-IP Cloudflare, `externalTrafficPolicy: Local`), cert-manager, PriorityClasses `mmm-prod`/`mmm-preprod`. **metrics-server non fonctionnel** (certificats kubelet sans IP SAN). **Prod ET preprod tournent** — constaté le 2026-08-28 : `mmm-prod` (api ×2, front ×2, postgres, redis, prometheus) et `mmm-preprod` (api, front, postgres, redis, pgadmin, prometheus) ; la mention « aucune release applicative déployée » datait du provisionnement et était devenue fausse. Réservations mesurées : prod 1 050 m / 1 728 Mi (limites 5 000 m / 6 400 Mi), preprod 450 m / 960 Mi, addons 400 m / 508 Mi. **Topologie de nœuds : appliquée le 2026-08-28** — `…-5sx65` porte le label et le taint `workload=tooling` (`k8s/node-topology.sh`) et n'héberge plus que SonarQube ; prod et preprod restent réparties sur `…-4g9mk` et `…-sccjf`. Les 4 pods applicatifs présents sur `…-5sx65` au moment du taint n'ont pas été expulsés (`NoSchedule`) et migreront à leur prochain rollout | 2026-08-28 |
| CD — deploy_prod (K8S_PROD_ENABLED) | **Toujours désactivé** (`false`) — 3ᵉ occurrence du même symptôme (après 2026-08-27/28) : les commits `6c3d02c`/`d86401c`/`d1ef46a` du 2026-08-31 (validateurs, mindmaps, formulaires) étaient poussés sur Docker Hub (`push_images` vert, images du 31/08 16:42 UTC) mais absents des pods — `helm history mmm-prod` figé en révision 4 du 27/08. Débloqué **manuellement** ce jour par `kubectl rollout restart deployment mmm-prod-api mmm-prod-front -n mymemomaster` (aucun changement Helm/ConfigMap requis, les 3 commits ne touchent que du code applicatif) — pods vérifiés sur les digests `sha256:1ce6f9d1…`/`sha256:80efe63e…` après rollout. Preprod vérifiée en parallèle : **pas de dérive** (pods preprod déjà sur les mêmes digests que Docker Hub), le seul écart est que la branche `staging` n'a reçu aucun merge depuis le 2026-07-19 (54 commits de retard sur `main`) — pas un bug CD. Cause racine non traitée : bascule de `K8S_PROD_ENABLED` sur GitHub (Settings → Variables) laissée à l'utilisateur, en cours | 2026-08-31 |
...
- `B4_RENDU.md` —
  ...
  - relecture stylistique (demande explicite « expression plus fluide et humaine ») : §7 « Pour dinir » → « Enfin » ; section 2 intro reformulée (deux phrases plates fusionnées, « quatre choses » → « quatre axes ») ; section 1 liste de dépendances restructurée (phrase finale détachée des puces) ; section 2 seuils bloquants (phrase redondante « si non respectés, le pipeline échoue » supprimée, fusionnée en amont) ; section 5 « évaluée en gain / coût » → « évaluée selon un rapport gain / coût » ; section 6 suppression de l'écho titre/premier mot (« journal des versions déployées » répété). Contenu technique inchangé.
  - ponctuation des énumérations harmonisée sur tout le dossier (demande explicite) : §1 espace avant `:` + ponctuation cohérente de la sous-liste (montées de version) ; §2 liste des 4 axes de supervision (retrait du point final isolé) ; §3.1 restructuration — le 3e tiret « La fiche… » n'appartenait pas à la même phrase que les 2 précédents, sorti en paragraphe séparé ; coquille « (en cours, validation validé) » corrigée en « (en cours, en validation, validé) » — à faire valider par l'utilisateur si ce ne sont pas les noms exacts des colonnes du kanban Odoo ; §4 virgule fautive « … livraison ainsi aucun déploiement… » corrigée en phrase à deux propositions ; annexe B espace manquante avant un lien corrigée.
  - §7 exemple forgot password (relecture après édition utilisateur) : 4e item de liste (compteur de tentatives) cassait le parallélisme fichier→description, sorti en phrase séparée ; répétition « Enfin… Enfin » sur deux paragraphes consécutifs, un des deux retiré ; virgule de juxtaposition « ne parvenait pas…, le parcours échouait » → deux-points ; même défaut trouvé et corrigé en §2 (« Ce verrou est volontaire, il permet… » → « … volontaire : il évite… »).
  - listes courtes reconverties en prose (demande explicite « transformer certains bullet points en paragraphes ») : §1 couches applicatives concernées (4 items → 1 phrase), §1 critères de revue des montées de version (sous-liste → clause au « je » à verbes actifs), §2 quatre axes de supervision (4 items → 1 phrase), §3.1 statut Odoo + avancée kanban (2 items → 1 phrase). Non touchés volontairement : fiches d'anomalie (§3.2, gabarit markdown), tableau des sondes, canaux de détection, toutes les annexes — contenu de référence à scanner, pas à lire en continu.
  - §2 seuils bloquants : l'utilisateur avait converti la liste en paragraphe mais la phrase avalait le verbe du 2e critère (backticks englobant toute la proposition au lieu du seul terme `healthy`) + virgule de juxtaposition ; reformulée avec deux-points et backticks limités au terme technique.
...
#### Ce qui est utilisable
- **Preprod déployée et fonctionnelle** (`mmm-preprod`, révision 1, 2026-08-26) : 6 pods `Running`, PVC Postgres lié en `csi-cinder-sc-delete`, certificats `api`/`front` émis. Vérifié de bout en bout à travers Cloudflare — `https://preprod.my-memo-master.com` → 200, `https://preprod-api.my-memo-master.com/api/v1/health` → 200 `{"status":"ok"}` (la readiness sonde la base, donc la connexion Postgres est validée).
- **Production déployée et fonctionnelle** (`mmm-prod`, révision 1, 2026-08-27) : 7 pods `Running`, les 2 replicas d'API et les 2 de front effectivement répartis sur des nœuds distincts (le `topologySpreadConstraints` fait son office), PDB actifs (`minAvailable: 1`), 3 PVC liés en `csi-cinder-sc-retain` (20+5+2 Gi), certificats `api`/`front` émis. Vérifié à travers Cloudflare — `https://app.my-memo-master.com` → 200, `https://api.my-memo-master.com/api/v1/health` → 200 `{"status":"ok"}`.
...
#### Dette / non couvert
- ~~Inscription en erreur 500 sur la prod~~ — **corrigé le 2026-08-27**. `EMAIL_FROM` valait `noreply@my-memo-master.com` alors que le compte SMTP authentifié est `support@my-memo-master.com` : Hostinger refusait l'expéditeur (`553 5.7.1 Sender address rejected: not owned by user`) et l'envoi de l'email de vérification faisait remonter un 500. `EMAIL_FROM` aligné sur `SMTP_USER` dans `helm/values.yaml`. Aucun compte orphelin créé : le controller supprimait déjà l'utilisateur en cas d'échec (`if (newUser) await userService.delete(...)`). Pour revenir à un `noreply@`, il faudra créer la boîte ou l'alias chez Hostinger et utiliser ses identifiants.
- **L'IP source vue par ingress-nginx est celle du load balancer Octavia (`172.21.2.240`), pas celle de Cloudflare.** Constaté le 2026-08-26 sur le déploiement preprod réel : `externalTrafficPolicy: Local` ne suffit pas, l'amphore Octavia fait elle-même du SNAT. Conséquence : `proxy-real-ip-cidr` (plages Cloudflare) ne matche jamais et les logs d'accès nginx affichent l'IP du LB. L'**application** n'est pas affectée — `clientIp()` lit `CF-Connecting-IP`, qui traverse intact, et `TRUST_PROXY_HOPS=2` retombe sur le bon client via la chaîne `X-Forwarded-For`. Correctif propre : activer le PROXY protocol sur Octavia (+ `use-proxy-protocol` côté nginx), ou ajouter le sous-réseau du LB (`172.21.0.0/16`) à `proxy-real-ip-cidr` — cette seconde option **uniquement après** le filtrage des plages Cloudflare par security group, sinon `X-Forwarded-For` devient falsifiable par quiconque joint l'origine en direct.
- ~~Certificat `mmm-preprod-pgadmin-tls` non émis~~ — **résorbé le 2026-08-27**. La finalisation ACME échouait sur `404 acme:error:malformed`, une erreur transitoire de Let's Encrypt alors que le challenge DNS-01 était déjà `valid`. Le retry automatique de cert-manager a abouti dans la nuit, sans intervention. À retenir : sur ce type d'échec, laisser le backoff faire son travail avant de forcer quoi que ce soit.
...
## [2026-08-28] [IMP] Déploiement réel de SonarQube sur le cluster + configuration de l'instance

**Contexte** — Les deux entrées précédentes livraient le chart et la topologie **sans les appliquer**. Le porteur du projet a demandé l'exécution. Tout ce qui suit a été fait sur le cluster `pck-dkoyol2`, qui héberge la production.

**Exécuté**
1. `bash k8s/node-topology.sh pck-dkoyol2-pgh-69qzm-5sx65` — label + taint `workload=tooling` posés.
2. `kubectl create namespace sonarqube` puis Secret `sonarqube-secrets` depuis `helm-sonarqube/secrets.env` (clés `PG_USER`, `PG_PASS`).
3. `helm upgrade --install sonarqube ./helm-sonarqube -n sonarqube --timeout 10m` — révision 1, `STATUS: deployed`.
4. Configuration de l'instance **par l'API**, tunnel `port-forward` ouvert : mot de passe `admin` changé (`POST /api/users/change_password`, HTTP 204), projet `entrezunfredici_MyMemoMaster` créé avec `mainBranch=main`, token `GLOBAL_ANALYSIS_TOKEN` nommé `github-actions-ci` généré puis validé (`/api/authentication/validate` → `{"valid":true}`).

**Écart assumé par rapport au runbook** — `--atomic` a été **retiré** du `helm upgrade`. Sur une première installation il supprime la release au moindre échec, donc efface les traces au moment précis où il faudrait lire les logs. À remettre pour les mises à jour suivantes.

**Ce que le déploiement a confirmé** — Trois hypothèses de conception, jusque-là non vérifiées, se sont révélées justes :
- **L'initContainer `privileged` est admis** : `kubectl logs -c init-sysctl` → `vm.max_map_count = 524288`, `fs.file-max = 131072`. L'échappatoire `esBootstrapChecksDisable` n'a pas servi.
- **L'initContainer `wait-postgres` a servi** : ses logs montrent `sonarqube-postgres:5432 - no response` avant `accepting connections`. Sans lui, le pod serait parti en CrashLoopBackOff.
- **`nodeSelector` + `tolerations` placent bien les deux pods** sur `…-5sx65`, et les 3 PVC se sont liés en `csi-cinder-sc-retain`.

**Vérifié** — `/api/system/status` → `{"id":"DC0EBA2E-…","version":"26.8.0.126808","status":"UP"}`. Logs : `SonarQube is operational`. Démarrage complet en ~100 s, bien en deçà du `failureThreshold: 60` (10 min) de la `startupProbe`.

**Point d'attention relevé** — `Ready` ≠ `UP` : le pod a été marqué `Ready` avant que SonarQube ne soit opérationnel, la sonde `httpGet` répondant 200 dès l'amorçage. C'est exactement la contrepartie documentée dans le template, et c'est ce qui justifie l'attente explicite de `"status":"UP"` dans le job CI.

**Fichiers créés (non commités, ignorés par git)**
- `helm-sonarqube/secrets.env` — `PG_USER` / `PG_PASS` (mot de passe généré, 28 caractères).
- `helm-sonarqube/credentials.local` — mot de passe `admin` de l'instance, valeur de `SONAR_TOKEN`, chemin du `KUBECONFIG_SONAR` encodé. Ajouté à `.gitignore`.

**Dette / reste à faire**
- **Les deux secrets GitHub ne sont pas posés** (`gh` n'est pas installé sur le poste) : `SONAR_TOKEN` et `KUBECONFIG_SONAR`. Tant qu'ils manquent, le job CI `sonarqube` **échouera au premier push sur `main`** — la branche `feat/sonarqube-k8s` ne doit pas être mergée avant.
...
### [2026-08-28] DOC — §7.1 du compte rendu actualisé sur les mesures réelles de l'instance SonarQube auto-hébergée

**Demande** : vérifier l'ajout de SonarQube et mettre le rapport à jour.

#### Vérifications faites
- **Chaîne CI opérationnelle** : job `sonarqube` de `ci.yml` (tunnel `kubectl port-forward` vers une instance ClusterIP, action `sonarqube-scan-action@v6`, limité à `main`), première analyse réelle en statut `SUCCESS` 135 s après le push (commit `d21f6df`).
- **Chart déployé** : `helm-sonarqube/` — SonarQube Community 26.8 + PostgreSQL 17 dédié sur `pck-dkoyol2`, namespace `sonarqube`, non exposé sur Internet.
- **Couverture toujours à 0 %, cause inchangée** : `test_and_lint` lance `npm run test` sans option de couverture, aucun artefact `lcov` n'est produit ni transmis, et `sonar.javascript.lcov.reportPaths` reste absent de `sonar-project.properties` (relu ce jour). **La migration ne corrige pas l'action P0.**
- **Accès direct à l'instance impossible depuis le poste** : le kubeconfig local pointe `pck-xolteoz` (`83.166.143.133:30922`), qui ne répond pas — autre cluster que celui hébergeant SonarQube, et injoignable. Les mesures sont donc reprises du relevé consigné dans ce journal, pas requêtées à nouveau. À rapprocher du risque n° 1 du §5.2 (secrets `KUBECONFIG_*` pointant l'ancien cluster) : **le même symptôme touche le poste de développement**.

#### Ce qui est écrit dans le rapport
`§7.1` réécrit : contexte de la migration, preuve que la chaîne est éprouvée, tableau comparatif des 9 métriques auto-hébergé / SonarCloud, cause de la couverture à 0 %, mesures locales de couverture, écart de sécurité, distinction avec le chiffre Dependabot, limites de l'édition Community. `§1` (ligne couverture), `§8` (tableau de progrès + deux actions P1) et l'annexe (source des mesures Sonar) alignés.

**L'écart de sécurité est traité comme un point ouvert, pas comme un chiffre.** 11 vulnérabilités sur l'instance contre 28 sur SonarCloud, alors que les 8 autres métriques concordent à quelques unités près et que les *security hotspots* sont à 0 des deux côtés. Le rapport énonce que **ni 11 ni 28 n'est citable** tant que l'écart n'est pas expliqué, et une action P1 dédiée (~0,5 JH) précède désormais celle d'ouverture des tickets. La note de sécurité **D** est identique des deux côtés et reste, elle, opposable.

#### Dette / points d'attention
- L'écart 11/28 est le point à instruire en premier : il conditionne tout énoncé chiffré sur la sécurité dans le dossier.
- La dette technique mesurée (4 743 min ≈ 79 h ≈ 9,9 JH) n'est portée par aucun ticket Odoo — elle n'entre donc pas dans les 634 h de reste-à-faire du §3.
- Le kubeconfig local à réparer si l'on veut pouvoir requêter l'instance depuis le poste plutôt que dépendre des relevés consignés.


---

## [2026-08-28] [FIX] Poste de développement : le kubeconfig par défaut pointait le cluster mort

**Symptôme** — Un agent tiers n'a pas pu interroger l'instance SonarQube : `kubectl` sans variable d'environnement visait `pck-xolteoz` (`https://83.166.143.133:30922`), pas le cluster de production `pck-dkoyol2`.

**Cause** — `~/.kube/config` datait du **28 avril** et portait encore l'ancien cluster. Toutes les commandes de cette session fonctionnaient parce qu'elles surchargeaient explicitement `KUBECONFIG=k8s/kubeconfig/pck-dkoyol2-kubeconfig` ; sans cette surcharge, `kubectl` tapait dans le vide. Vérifié : `pck-xolteoz:30922` ne répond plus du tout (HTTP 000), `pck-dkoyol2:31159` répond 200. Les deux clusters partagent la même IP publique et ne se distinguent que par le port — ce qui rend la confusion facile et silencieuse.

**Corrigé**
- `~/.kube/config` remplacé par le kubeconfig de `pck-dkoyol2`. `kubectl` fonctionne désormais **sans** `export KUBECONFIG`. Ancien fichier sauvegardé hors dépôt.
- `~/.kube/cache` purgé (cache de découverte de l'API de l'ancien cluster).
- **Deux copies base64 du kubeconfig supprimées** : `k8s/kubeconfig/sortie.txt` et `helm-sonarqube/KUBECONFIG_SONAR.b64`, découvertes **strictement identiques** (7 524 octets, `cmp` concluant). C'est un identifiant d'administration du cluster : le garder en trois exemplaires sur le disque n'apporte rien puisqu'il se régénère en une commande (`base64 -w0 k8s/kubeconfig/pck-dkoyol2-kubeconfig`). Il ne reste qu'une source de vérité : `k8s/kubeconfig/pck-dkoyol2-kubeconfig`.
...
## [2026-08-30] [IMP] Job QA vert sur le runner — les trois tâches sont réellement rejouées

**Résultat** — Le job `e2e_and_load` passe au vert sur le runner GitHub (commit `71ce5ee`, 4 min 24 s). Le run complet est vert : `setup`, les deux `test_and_lint`, `SonarQube Analysis` (1 min 13 s) et le job QA. Les annotations Playwright confirment les deux suites — **8 passed** pour l'audit de contraste, **5 passed** pour les parcours E2E.

**Ce que ça change** — Les tâches `QA.03`, `QA.05` et `QA.06` passent de « vérifiées une fois sur un poste » à « rejouées à chaque passage sur `main` et `staging` ». C'est la différence que visait le niveau de preuve demandé.

**Quatre échecs successifs ont été nécessaires, et chacun a appris quelque chose**

1. **Vitest ramassait `e2e/`** — la config excluait `e2e-a11y/**` mais pas le nouveau répertoire. Créer un dossier de specs sans rejouer la suite unitaire laisse passer exactement ce défaut.
2. **ESLint sur `process`** — corrigé par un `overrides` déclarant l'environnement Node, plutôt qu'un `eslint-disable` par fichier que le fichier suivant aurait oublié.
3. **`.env.ci` non transmis au shell Playwright** — hypothèse partiellement juste, correctif insuffisant.
4. **`.env.ci` sourcé en bloc** — `VITE_APP_DESCRIPTION=Stack CI ephemere`, valeur non quotée avec espaces : `.` traite un `.env` comme du shell, d'où `CI: command not found` et exit 127. **Ce défaut était reproductible en local sans Docker ni CI** — un `set -a; . ./.env.ci` suffisait. Il a coûté un run pour être appris.
...
## [2026-08-31] IMP — Réédition du compte rendu de pilotage : premier arrêté où le registre Odoo n'a strictement pas bougé

**Contexte** — Demande utilisateur : mettre à jour `docs/COMPTE_RENDU_METRIQUES.md`. Depuis l'arrêté du 2026-08-30, trois commits ont été livrés en session (validateurs 50 caractères, bugs carte mentale, fermeture des modales + redirection Leitner) — aucun n'est rattaché à une tâche du registre Odoo.

**Ce qui a été revérifié, bon marché** : extraction Odoo fraîche (`odoo_cli.py read project.task`, 350 tâches) — **résultat identique octet pour octet** à l'arrêté du 2026-08-30 sur les cinq indicateurs qui en dépendent (Avancement 217/248, Coûts 108 675 €/86 250 €, Délais 30 tâches en retard, Risques 183/91/78, RH 362,3 JH) ; recherche par mot-clé des trois commits dans les 350 tâches, aucune correspondance. Suites de tests rejouées : API 1 560/1 560 (+6), front 703/703 (+14), coverage API 81,4 % identique / front 59,75 % (+0,85 pt). RGAA revérifié sans régression : `node scripts/audit-a11y.mjs` → 0/79, `npx vitest run test/a11y/axe.test.js` → 20/20 (y compris `ModalComponent.vue`, dont le mécanisme de fermeture a changé le 2026-08-31). `npm audit --omit=dev` rejoué (API + front) → 0/0, absent du rapport précédent. État CI des 3 commits vérifié via l'API GitHub (`/commits/<sha>/check-runs`) : tous verts sur `test_and_lint` ×2, `SonarQube Analysis`, `Parcours E2E + charge` ; `Build and Push Docker Images` du dernier commit encore `in_progress` au moment de la rédaction — signalé tel quel, pas attendu indéfiniment.

**Ce qui reste daté, cher à revérifier** : couverture et qualité SonarQube détaillées toujours reprises de l'analyse du 2026-08-29 (`SONAR_TOKEN` absent de l'environnement local, vérifié) — désormais antérieures à 4 commits dont les analyses ont pourtant toutes réussi (vert en CI) sans que leurs valeurs aient pu être relues. Audit manuel des 106 critères RGAA vérifié inchangé (23/106) : aucune session dédiée à sa poursuite dans cet intervalle, pas de faux travail affiché.

**Nouveau constat de méthode, documenté pour la suite** : le registre Odoo ne capture par nature aucun correctif signalé et traité directement en session hors ticket — ce n'est pas un défaut de mesure à corriger dans ce rapport, mais un angle mort à garder explicite d'un arrêté à l'autre plutôt que de laisser croire que « rien n'a bougé dans le registre » signifie « rien n'a été livré ». Voir `DECISIONS.md`.

**Fichiers modifiés**
- `docs/COMPTE_RENDU_METRIQUES.md`

**Vérifié** : voir le détail des commandes rejouées ci-dessus et dans l'annexe du rapport lui-même (§ « reproductibilité des mesures », mise à jour dans le même geste).

---

## [2026-08-31] IMP — Audit RGAA 106 critères : 23/106 → 60/106, 5 non-conformités confirmées (dont 1 sévère)

**Contexte** — Demande explicite de l'utilisateur de reprendre l'audit manuel des 106 critères RGAA (`docs/AUDIT_RGAA_106.md`, ouvert le 2026-08-30, resté à 23/106 depuis) : « es-ce que tu peut t'en occuper car moi je sais pas faire ». Pas de session Playwright/navigateur cette fois — uniquement lecture de code, `grep` exhaustif dans `src/`, et lecture de fichier par fichier.

**Ce qui a été fait** — Thématiques 1 (Images, 9/9), 5 (Tableaux, 8/8) et 6 (Liens, 2/2) closes ; progrès partiels sur 4, 8, 9, 11, 12, 13 (voir tableau détaillé dans `docs/AUDIT_RGAA_106.md`). **5 non-conformités confirmées par preuve directe** (fichier/ligne ou recherche exhaustive `grep -rn`), pas des candidats à confirmer :

1. **6.2 (sévère)** — La navigation principale de l'application (`App.vue`, blocs desktop lignes 11-65 et mobile lignes 99-150, ~11 `router-link` chacun) n'a **aucun nom accessible** sur 8 des ~11 liens (accueil, tutoriels, cartes mentales, flashcards, exercices, classe, profil, réglages) : contenu = icône seule (Heroicons ou SVG maison sans `<title>`/`aria-label`), sans texte ni `aria-label` ni `title` sur le lien lui-même. Seuls 3 liens (`/calendar`, `/todo`, `/kpi`) ont un `title` (mécanisme faible). **Invisible à l'outillage existant** : `App.vue` n'est couvert par aucun des 20 tests axe-core (qui ciblent des composants isolés sous `test/components/`, pas le shell applicatif) — exactement le type d'angle mort que cet audit manuel est censé révéler.
2. **5.4-5.7** — Les 4 `<table>` du dépôt (`AdminPage.vue` mort, `ClassroomPlateformeView.vue`, `CreditsPage.vue`, `ExerciseDetailPage.vue`) n'ont ni `<caption>` ni `scope` sur leurs `<th>` (`grep -rn '<caption\|scope='` → 0 résultat).
3. **11.6/11.7** — Aucun `<fieldset>`/`<legend>` dans tout `src/` (`grep -rn '<fieldset\|<legend'` → 0), alors que de vrais regroupements existent : QCM à choix radio (`ExerciseDetailPage.vue`, `ExercisesPage.vue`, `FlashcardsCardsPage.vue`, `FlashcardsSessionPage.vue`), sélection de jours par cases à cocher (`ClassroomEtablissementView.vue`). Chaque option a bien un `<label>` individuel (11.1/11.4 OK), mais l'énoncé du regroupement (ex. la question du QCM) n'est associé par aucun mécanisme programmatique.
4. **11.13** — `autocomplete` absent des champs email/mot de passe de connexion (`ConnexionPage.vue`), inscription (`InscriptionPage.vue`) et vérification d'email (`VerifyEmailPage.vue`) — seulement 2 usages dans tout le dépôt (`ProfilePage.vue`, `ResetPasswordPage.vue`).
5. **1.2** — `AuthFormLayout.vue` : illustration décorative avec `alt="Illustration"` (valeur par défaut de la prop `imageAlt`, jamais surchargée) au lieu de `alt=""`, sur les 5 pages d'authentification.

**Effet de bord relevé en marge** (pas RGAA) : le lien « Classe » de la nav mobile (`App.vue:121`) réutilise par erreur `<ExercisesIcon>` au lieu de `<ClassroomIcon>` (utilisé côté desktop) — bug cosmétique, signalé mais non corrigé ici.

**Correction de méthode, en passant** : le tableau « État global » du document (compteur de verdicts posés par thématique) était désynchronisé de la grille détaillée pour les thématiques 4, 12 et 13 depuis les sessions précédentes — des verdicts déjà posés dans la grille (ex. `12.10`, `13.5/13.6/13.8/13.10/13.12`) n'avaient jamais été reportés dans le compteur. Recompté directement depuis la grille pour tout le document, pas seulement les thématiques touchées cette session — c'est ce qui explique une partie du saut 23→60 au-delà du travail de cette session.

**Fichiers modifiés**
- `docs/AUDIT_RGAA_106.md`
- `docs/COMPTE_RENDU_METRIQUES.md` (chiffres RGAA mis à jour post-arrêté, voir entrée précédente)

**Vérifié** : chaque verdict posé est sourcé (chemin de fichier + ligne, ou commande `grep` exhaustive citée) — aucune estimation. Pas de rejeu de test automatisé nécessaire (travail de lecture de code, aucun fichier applicatif modifié).

**Dette signalée, non traitée ici** : les correctifs des 5 non-conformités confirmées n'ont pas été appliqués — l'utilisateur a demandé la poursuite de l'audit, pas encore les correctifs (à proposer explicitement). 46 critères restent à auditer, dont ceux qui exigent un rendu de page réel (Playwright) : zoom 200/400 %, ordre de tabulation, focus visible, clignotements, orientation d'écran — aucun test de ce type n'a été fait cette session.
...
## [2026-08-31] FIX — Prod figée en révision 4 malgré 3 commits mergés ; preprod vérifiée saine

**Contexte** — Signalement utilisateur : les derniers développements mergés sur `main` (`6c3d02c`, `d86401c`, `d1ef46a`) ne se reflétaient pas en prod. Diagnostic mené avec un accès direct disponible ce jour au cluster (`kubectl` déjà configuré sur le contexte `kubernetes-admin@pck-dkoyol2` du poste), absent des sessions précédentes qui n'avaient pu que déduire l'état de la prod depuis le dépôt.

**Cause — 3ᵉ occurrence du même défaut déjà consigné les 2026-08-27/28** : `deploy_prod` de `cd.yml` est conditionné à `vars.K8S_PROD_ENABLED == 'true'`, resté à `false`. Vérifié en direct : `push_images` avait bien fonctionné (images `fredissimo/mymemomaster_api:latest`/`_front:latest` poussées sur Docker Hub le 2026-08-31 à 16:42 UTC, digests `sha256:1ce6f9d1…`/`sha256:80efe63e…`), mais `helm history mmm-prod -n mymemomaster` restait figé en **révision 4 du 27/08 11:35** et les pods tournaient sur `sha256:31361540a…` (digest du 28/08, antérieur aux 3 commits). Le dépôt était juste, la CD build/push fonctionnait, seul le déploiement effectif manquait.

**Corrigé en ligne (autorisé par le porteur du projet, action + variante confirmées via question posée)**
- `kubectl rollout restart deployment mmm-prod-api mmm-prod-front -n mymemomaster` — pas de `helm upgrade` nécessaire : les 3 commits ne touchent que du code applicatif (`my_memo_master_front/src/**`, `my_memo_master_api/validators/**`), aucun fichier `helm/`/`k8s/`, donc aucune dérive de ConfigMap à craindre contrairement à l'incident du 28/08.
- Vérifié après rollout : `kubectl rollout status` conclusif sur les deux deployments, digests des nouveaux pods relevés et comparés à Docker Hub (`sha256:1ce6f9d1…` API, `sha256:80efe63e…` front — match exact), hash du bundle front visiblement changé (`index-CUX8KcdE.js` → `index-BCKGAq6i.js` sur `GET /`).

**Preprod vérifiée par la même occasion (question explicite de l'utilisateur)** — Pas le même problème : `helm history mmm-preprod -n mymemomaster-preprod` est aussi figé (révision 7 du 27/08), mais les pods tournent déjà sur les digests actuellement publiés sur Docker Hub (`mymemomaster_preprod_api/front:latest`, `sha256:2c38c1c8…`/`sha256:b583f44e…`, poussés le **2026-07-19**) — **aucune dérive déployé/poussé**. La staleness vient d'ailleurs : la branche `staging` n'a reçu aucun merge depuis le **2026-07-19** (`01d2f99`), soit **54 commits de retard** sur `main` (`staging` reste un ancêtre direct de `main`, pas de divergence de code). `K8S_PREPROD_ENABLED` est également à `false` mais n'a rien changé récemment puisque rien de neuf n'a été poussé sur `staging` à déployer. Aucune action prise sur preprod, à la demande explicite de l'utilisateur (« je gère »).

**Utilisable** — Prod à jour avec `main` (commit `d1ef46a` inclus) sans action supplémentaire.

**Vérifications** — `kubectl rollout status` (2 deployments) ✅ · digests des pods comparés à Docker Hub ✅ · hash du bundle front changé sur `GET https://app.my-memo-master.com/` ✅ · `helm history`/`kubectl get pods -o wide` relus sur les deux namespaces (`mymemomaster`, `mymemomaster-preprod`) · comparaison staging/main via `git merge-base --is-ancestor` et `git log origin/staging..origin/main --oneline`.

**Hypothèses posées**
- Le poste dispose d'un accès `kubectl` valide et à jour vers `pck-dkoyol2` (contexte déjà présent avant cette session, non recréé ici) — à ne pas supposer disponible dans une session future sans le revérifier (`kubectl config current-context`).
- Digests Docker Hub interrogés via l'API publique `hub.docker.com/v2` (pas de token nécessaire, dépôt/images publics) — fiable pour dater un push mais ne remplace pas `gh run list` (toujours indisponible sur ce poste, `gh` non installé) pour lire l'issue réelle du job CI.

**Dette / reste à faire**
- **`K8S_PROD_ENABLED` reste à `false`** — cause racine non traitée, laissée à l'utilisateur (bascule via GitHub Settings → Secrets and variables → Actions → Variables, hors accès `gh`/token depuis ce poste). Tant qu'elle n'est pas activée, tout futur merge sur `main` retombera dans le même symptôme et devra être rejoué à la main.
...
## [2026-09-01] FIX — 500 en prod à la création d'une série d'exercices (`testQuestions` sans timestamps réels) + 4 tables de jointure au même défaut latent corrigées préventivement

**Contexte** — Signalement utilisateur (screenshot page 500 pleine page) confirmé en direct : « j'ai une erreur bien réelle au moment de faire les séries d'exercices ». Accès `kubectl` disponible ce jour (contexte `kubernetes-admin@pck-dkoyol2`) — diagnostic mené directement sur les logs et métriques du cluster prod plutôt que par déduction.

**Cause** — `Test.belongsToMany(Question, { through: 'testQuestions', ... })` et l'association inverse (`Question.model.js`) passent `through` en simple chaîne, sans modèle Sequelize enregistré pour cette table de jointure. Sequelize crée alors un modèle implicite avec `timestamps: true` par défaut (aucun `define.timestamps` global dans `dbms.config.js`) — il s'attend à des colonnes `createdAt`/`updatedAt` sur `testQuestions`. Or la migration d'origine (`20260226152800-create-testquestions-table.js`) n'a créé que `idTest`/`idQuestion`. Résultat : `question.addTest(test)` dans `QuestionService.create()` génère un `INSERT` avec des colonnes inexistantes → 500 Postgres (`column "createdAt" does not exist` / `column question->testQuestions.createdAt does not exist`, confirmé dans les logs des deux pods `mmm-prod-api-*`).

**Pourquoi seulement en prod** — `server.js` ne lance `syncModels()` (donc `instance.sync({ alter })`, qui aurait ajouté les colonnes manquantes pour matcher le modèle implicite) que si `ENVIRONMENT !== 'prod'` **ou** base vide. En dev/preprod, ce sync silencieux a toujours fait converger le schéma réel vers l'attente implicite de Sequelize (colonnes `createdAt`/`updatedAt` ajoutées automatiquement) — masquant le défaut. En prod (`ENVIRONMENT=prod`, sync désactivé), seule la migration fait foi : le désaccord modèle/table réelle n'a jamais été corrigé, et n'était visible que là.

**Confirmé via métriques Prometheus** (`http_requests_total`, label `status_code`, pas `code`) : exactement **1 requête `POST /api/v1/questions/` en 500 sur les dernières 24h**, aucune autre route en 5xx sur la même fenêtre — cohérent avec un incident isolé à la 1ʳᵉ question créée après le `POST /tests` réussi (d'où le message de succès affiché juste avant le crash).

**Bug latent identique trouvé en creusant le même pattern (`through: 'string'` sans modèle enregistré)** — grep systématique de tous les `through:` du projet : 5 tables de jointure sur 9 n'ont pas de modèle explicite enregistré (contre `TestClassGroup`, correctement modélisé avec `timestamps: false`, qui sert de référence) :
- `testQuestions` (Test↔Question) — **celui qui a crashé**
- `TestTag` (Test↔Tag) — même défaut, pas encore déclenché (aucun tag sélectionné dans l'incident observé)
- `questionSubject` (Question↔Subject)
- `cardSystems` (LeitnerCard↔LeitnerSystem)
- `MindMapTag` (Diagramme↔Tag), `LeitnerSystemTag` (LeitnerSystem↔Tag)

Les 5 migrations correspondantes ont été vérifiées une à une : aucune ne crée `createdAt`/`updatedAt`, même défaut confirmé partout.

**Décision** — Corriger les 5 tables en une fois (pas seulement `testQuestions`), par le même correctif mécanique et déjà éprouvé par `TestClassGroup.model.js` : un modèle Sequelize explicite par table de jointure, `timestamps: false`, enregistré dans `models/index.js`. Pas de migration nécessaire — le schéma physique de chaque table (prod) correspond déjà à ce que `timestamps: false` attend ; le correctif aligne uniquement la couche ORM sur la réalité de la table, il ne la modifie pas.

**Ce qui a été fait**
- `models/TestQuestion.model.js`, `models/TestTag.model.js`, `models/QuestionSubject.model.js`, `models/CardSystem.model.js`, `models/MindMapTag.model.js`, `models/LeitnerSystemTag.model.js` — 6 nouveaux modèles, `timestamps: false`, colonnes alignées sur leurs migrations respectives.
- `models/index.js` — les 6 enregistrés (`Test.model.js`/`Question.model.js`/`Tag.model.js`/`Subject.model.js`/`LeitnerSystem.model.js`/`LeitnerCard.model.js`/`Diagramme.model.js` inchangés : ils référencent déjà `through: 'nomDeTable'`, qui matche désormais le modèle enregistré au lieu d'en créer un implicite).

**Vérifié** : `npx jest` (API) → **1560/1560**, 0 régression, y compris `test/bdd/exercise.session.test.js` (utilise `syncModels({force:true})`, donc sensible à un modèle mal défini) et les controllers Test/Question/Tag. Confirmé sur le cluster prod : logs des 2 pods API relus sur toute leur durée de vie (aucune nouvelle erreur `createdAt` après correctif — correctif pas encore déployé au moment de la vérification, donc lecture de référence avant/après à refaire post-déploiement), requêtes Prometheus KPI (`/api/v1/kpi/my`, `/api/v1/kpi/alert-settings`) confirmées saines sur 24h à la demande explicite de l'utilisateur (aucun 5xx, uniquement 200/304/401).

**Fichiers modifiés**
- `my_memo_master_api/models/TestQuestion.model.js` (créé)
- `my_memo_master_api/models/TestTag.model.js` (créé)
- `my_memo_master_api/models/QuestionSubject.model.js` (créé)
- `my_memo_master_api/models/CardSystem.model.js` (créé)
- `my_memo_master_api/models/MindMapTag.model.js` (créé)
- `my_memo_master_api/models/LeitnerSystemTag.model.js` (créé)
- `my_memo_master_api/models/index.js`

**Dette signalée, non traitée ici**
- **Le correctif n'est pas déployé** — modifications faites sur le poste local uniquement (branche `main`, non commit à la demande implicite : aucun commit/push effectué dans cette session). Reste à commit, merge et laisser la CD déployer (cf. l'historique récurrent de `K8S_PROD_ENABLED` — vérifier que le déploiement prod se déclenche bien avant de considérer l'incident clos côté utilisateurs).
- **Effet de bord front repéré mais non corrigé** : `helpers/api.js` fait un `router.push('/error-server')` **inconditionnel** dans le `catch` de `get`/`post`/`put`/`del`/`patch` dès qu'une requête sort de la plage `validateStatus` (200-499) — n'importe quel vrai 500, même sur un appel secondaire après un succès principal (ex. la 1ʳᵉ question d'une série après le test déjà créé), fait basculer toute l'app sur la page d'erreur pleine écran et perd le contexte en cours. Ce comportement a amplifié la sévérité perçue de ce bug (le test était bien créé, mais l'utilisateur n'a vu qu'un écran d'erreur) ; non corrigé car hors périmètre de la demande (le signalement portait sur la cause du 500, pas sur la gestion d'erreur front) — à proposer séparément si des 500 partiels doivent un jour dégrader l'UX plus proprement qu'un écran plein page.
- **`questionSubject`/`cardSystems`/`MindMapTag`/`LeitnerSystemTag`/`TestTag` corrigés préventivement mais jamais reproduits en prod** — le correctif élimine un bug de classe identique avant qu'il ne se déclenche (mêmes conditions que `testQuestions` : écriture sur la table de jointure + prod jamais resynchronisée), mais aucune preuve de log ne confirme qu'ils auraient effectivement crashé un jour ; risque jugé suffisamment net (5 tables, même pattern exact, même absence de colonnes) pour ne pas attendre un 6ᵉ incident avant de corriger.

**Effet de bord front traité dans la foulée (demande explicite de l'utilisateur)** — `helpers/api.js` redirigeait inconditionnellement vers `/error-server` (page pleine écran) dans le `catch` de `get`/`post`/`put`/`del`/`patch` dès qu'un vrai 5xx ou une erreur réseau survenait, **y compris sur un appel secondaire après un succès déjà affiché** (exactement le cas de cet incident : le `Test` était créé avec succès, la 1ʳᵉ `Question` a crashé en 500, et l'utilisateur s'est retrouvé sur une page d'erreur plein écran ayant perdu tout contexte). Retiré : les 5 fonctions retournent désormais simplement `undefined` (comportement déjà existant, la redirection n'était qu'un effet de bord additionnel) et loguent l'erreur en console — chaque appelant gère déjà ce cas via son propre `try/catch`/`notif.notify` ou une garde `if (!resp) …`, pattern déjà systématique dans les stores (vérifié sur `tests.js`, `tags.js`, `ExercisesPage.vue`). `import router from '@/router'` retiré (devenu inutilisé). Route `/error-server` et `ErrorServerPage.vue` **conservées mais désormais orphelines** (plus aucun appelant ne les déclenche) — non supprimées, hors périmètre de cette demande, laissées disponibles si un futur usage plus ciblé (ex. erreur réseau totale sur le chargement initial de l'app) est décidé.

**Fichiers modifiés (effet de bord front)**
- `my_memo_master_front/src/helpers/api.js`
- `my_memo_master_front/test/helpers/api.test.js` (4 tests renommés/adaptés : plus d'assertion sur `router.push`, vérifient `console.error` + `undefined`)

**Vérifié** : `npx vitest run` (front) → **708/708**, 0 régression ; `npx eslint src/helpers/api.js test/helpers/api.test.js` → 0 erreur ; grep sur `test/` confirmant qu'aucun autre test ne référence `/error-server` ou `mockRouterPush`.
