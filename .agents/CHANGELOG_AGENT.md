...
| Audit RGAA 4.1.2 — les 106 critères | **EN COURS** (ouvert 2026-08-30, poursuivi 2026-08-31 — **57 % posé, 60/106 verdicts**) — `docs/AUDIT_RGAA_106.md`. Thématiques **1 (Images), 5 (Tableaux), 6 (Liens) closes** ; progrès partiels sur 4, 8, 9, 11, 12, 13 ; 3, 7, 10 encore ouvertes. **5 non-conformités confirmées** (pas de simples candidats) en plus des 2 déjà connues (8.6, 12.7) : **6.2 sévère** — navigation principale de l'app (`App.vue`) sans nom accessible sur 8 liens/11 (icônes seules) ; 5.4-5.7 — aucun `<caption>`/`scope` sur les 4 tableaux de données ; 11.6/11.7 — aucun `<fieldset>`/`<legend>` dans tout le dépôt malgré de vrais regroupements (QCM radio) ; 11.13 — `autocomplete` absent des formulaires d'authentification ; 1.2 — image décorative avec `alt="Illustration"` au lieu de `alt=""`. Toutes vérifiées par lecture de code + recherche exhaustive, pas de test sur page rendue cette session | 2026-08-31 |
