...
- **A02 — Cryptographie** : mots de passe bcrypt ; access token 15 min ; refresh token opaque avec **rotation à chaque renouvellement** ; code de réinitialisation à 6 chiffres **hashé bcrypt** en base (le code brut ne transite que par email), sa faible entropie étant compensée par une expiration de 15 min, **5 essais max**, un usage unique et la révocation du refresh token après réinitialisation ;
