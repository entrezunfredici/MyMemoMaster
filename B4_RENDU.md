...
**Projet : MyMemoMaster** est une plateforme web de révision et de suivi pédagogique
...
- **Systèmes de Leitner** : questions-réponses à répétition espacée où l'algorithme représente les cartes dans des « boîtes » et fait remonter plus fréquemment les questions échouées et moins fréquemment les questions réussies ;
...
**Second exemple réel — dépendance tierce non maintenue (2026-08-15)** : `npm audit --omit=dev --audit-level=high` faisait échouer le CI sur `protobufjs` (critique) et `sharp` (haute), tous deux embarqués en dur par `@xenova/transformers` (moteur de correction sémantique) via `onnxruntime-web` → `onnx-proto`. `@xenova/transformers` était déjà à sa dernière version publiée (2.17.2, aucune 3.x n'existe sous ce nom de paquet) : impossible d'attendre une montée de version amont. `npm audit fix --force` ne proposait qu'une **rétrogradation** vers `@xenova/transformers@1.4.2`, antérieure à la version installée — inutilisable sur une dépendance qui porte une fonctionnalité cœur du produit. J'ai forcé `protobufjs`/`sharp` (et `uuid`, même défaut chez `sequelize`) à une version corrigée dans tout l'arbre via le champ `overrides` de `package.json`, sans toucher aux paquets qui les embarquent. Comme `@xenova/transformers` est mocké globalement en test (`moduleNameMapper` Jest, le modèle réel n'est jamais chargé pendant `npm test`), une suite verte n'aurait rien prouvé sur la compatibilité réelle : j'ai donc chargé le vrai modèle (`pipeline('feature-extraction', 'Xenova/paraphrase-multilingual-MiniLM-L12-v2')`, celui utilisé en production) hors suite de tests et calculé un embedding avec succès, avant de rejouer la suite complète (1530 tests) et de vérifier les versions réellement installées dans l'image Docker reconstruite. `npm audit --omit=dev` : 0 vulnérabilité.
...
| Sonde d'uptime externe          | [.github/workflows/uptime.yml](.github/workflows/uptime.yml)             | Depuis l'infrastructure GitHub (donc hors du VPS/cluster supervisé), ping de`/api/v1/health` toutes les 5 minutes avec 3 tentatives espacées de 10 s pour absorber un incident transitoire, alerte Discord en cas d'échec confirmé. URLs sondées configurées par la variable de dépôt `UPTIME_URLS`.                          |
| Alertmanager sur métriques     | [helm/templates/alertmanager.yaml](helm/templates/alertmanager.yaml)     | Alertes évaluées par le Prometheus du chart sur les métriques RED/USE de l'API : cible injoignable (2 min), taux de 5xx > 5 % (5 min), latence p95 > 1 s (10 min), event loop Node.js > 500 ms (5 min), ces seuils sont ajustables par values. Routées vers le même webhook Discord, relance toutes les 4 h tant que l'alerte est active. |
...
*Capture réelle d'une notification Discord de fin de déploiement sur `staging` avec tests verts, résumé de l'état du cluster (nœuds, conditions kubelet, pods par statut) et confirmation « Déploiement staging réussi ».*
...
*Capture réelle du signalement automatique du premier canal signalant l'échec des tests sur la branche `dev_back_student_kpi`, notifié sur le canal Discord du projet à la fin du run CI.*
...
**R1 : Tags git semver + images Docker taguées (réalisée).** Les images étaient poussées en `:latest` par environnement ; le rollback VPS demandait d'« identifier le tag de l'image précédente sur DockerHub », impossible avec un tag unique écrasé. Un nouveau job [`tag_release`](.github/workflows/cd.yml) (staging/main uniquement, en parallèle des jobs de déploiement — un échec ne bloque jamais une mise en prod) calcule à chaque merge le prochain tag `AAAA.MM.n` (convention déjà utilisée par ce journal, section 6 ; `staging-AAAA.MM.n` pour staging afin d'éviter toute collision de tag git avec `main`), re-tague les images Docker Hub déjà poussées via `docker buildx imagetools create` (pas de rebuild, même digest), crée le tag git correspondant et publie une Release GitHub via le `gh` CLI déjà présent sur les runners (aucun nouveau secret ni Action tierce). Gain : rollback fiable en ciblant explicitement une version au lieu du `:latest` mouvant.

**R2 : Persistance Redis pour les rappels BullMQ (réalisée).** Redis tournait sans persistance AOF/RDB (décision documentée le 2026-06-12), ainsi un redémarrage du conteneur perdait tous les rappels programmés, silencieusement. `--appendonly yes` activé sur le service `redis` de [docker-compose.yml](docker-compose.yml) (volume `redis-data` déjà monté) ; le chart Helm de prod l'avait déjà côté StatefulSet ([helm/templates/redis.yaml](helm/templates/redis.yaml)). Gain : fiabilité d'une fonctionnalité cœur de la promesse produit, pour un coût de configuration négligeable.

**R3 : Alerting proactif sur la disponibilité (réalisée).** La supervision détectait mais n'alertait pas en dehors des déploiements (section 2), ainsi une panne entre deux déploiements n'était vue qu'au prochain usage. Les deux volets de la recommandation ont été **mis en œuvre**. Premier volet : une sonde d'uptime externe ([.github/workflows/uptime.yml](.github/workflows/uptime.yml)) pingue `/api/v1/health` toutes les 5 minutes depuis l'infrastructure GitHub, qui est donc indépendante du VPS et du cluster supervisés, avec 3 tentatives espacées de 10 s pour absorber les incidents transitoires et alerter sur le webhook Discord déjà en place ; détection ramenée à ≤ 10 min (granularité du cron GitHub incluse) sans nouvel outil d'équipe. Second volet : un Alertmanager ([helm/templates/alertmanager.yaml](helm/templates/alertmanager.yaml)) branché sur le Prometheus du chart évalue des règles sur les métriques RED/USE de l'API, cible injoignable, taux de 5xx, latence p95, saturation de l'event loop et route vers le même webhook Discord ; désactivé par défaut (`monitoring.alerting.enabled`), son activation exige la clé `DISCORD_WEBHOOK_URL` dans le Secret du namespace (section 2). Les deux vues sont complémentaires : la sonde observe le service de l'extérieur (up/down), l'Alertmanager de l'intérieur (dégradations avant la panne).
...
**R5 : Cache des droits Leitner (réalisée par anticipation).** Chaque écriture sur une carte déclenchait 1-2 requêtes DB de résolution de droits ([my_memo_master_api/services/LeitnerCard.service.js](my_memo_master_api/services/LeitnerCard.service.js), `resolveUserRights`) ; la recommandation avait initialement été différée jusqu'à un besoin de charge mesuré. Réalisée sur demande, sans attendre ce signal : cache Redis clé/valeur (`leitner:rights:<userId>:<idSystem>`, TTL 30 s) doublé d'une **invalidation explicite** à chaque écriture de partage (`LeitnerSystemsUsers.create/update/delete`, `LeitnerSystem.share`) — la TTL courte n'est qu'un filet de sécurité, pas le mécanisme de fraîcheur principal, un partage retiré est donc immédiatement pris en compte dans le cas nominal. Client Redis dédié « fail-fast » (échec en moins d'1 s si Redis est injoignable plutôt que d'attendre indéfiniment, comme pour la révocation JWT ci-dessus) : une panne Redis dégrade vers la résolution DB déjà en place avant ce ticket, jamais un blocage. L'endpoint `requireRole` (rôle plateforme) n'est pas concerné — périmètre volontairement limité aux droits Leitner explicitement visés par R5.
...
Une « version » correspond ici à un jalon mergé sur une branche de déploiement (`dev` → test, `staging` → preprod, `main` → prod), nommée `AAAA.MM.n` (année.mois.itération). Les versions antérieures à la réalisation de la recommandation R1 (§5) sont reconstituées a posteriori faute de tags git ; depuis, un tag git et une Release GitHub portant ce même nom sont créés automatiquement à chaque merge sur `staging`/`main` ([.github/workflows/cd.yml](.github/workflows/cd.yml), job `tag_release`). Le détail technique exhaustif de chaque livraison reste dans [dev/CHANGELOG.md](annexes/dev/CHANGELOG.md) ; ce journal en est la vue synthétique orientée exploitation.

**2026.08.1 : Résorption de dette sécurité/fiabilité + recommandations R1/R2** (2026-08-15). Nouvelles fonctionnalités : révocation JWT (blacklist Redis par cutoff `iat` utilisateur, `helpers/tokenBlacklist.js`), branchée sur déconnexion, changement/réinitialisation de mot de passe et désactivation de compte (A07-M1) ; tagging de version automatisé `AAAA.MM.n` + Release GitHub sur staging/main (R1, §5) ; persistance Redis AOF activée (R2, §5). Anomalies corrigées : `MAX_USERS` n'était transmis à aucun conteneur Docker ni à la ConfigMap Helm depuis son introduction (limite d'inscriptions silencieusement inactive) ; `isRegistrationOpen` comptait les comptes désactivés (désactiver un compte ne libérait pas de place) ; `OnboardingState.service` levait une exception au lieu de retourner `null` (500 au lieu de 404 pour les comptes legacy) ; `_processPendingEmailInvitations` sans transaction ni révocation de l'ancien gérant établissement ; image Docker de l'API — CVE critique sur le `tar` embarqué par npm corrigée (`npm install -g npm@latest`) ; `npm audit` CI — CVE critique (`protobufjs`) et haute (`sharp`) corrigées par `overrides` ciblés (§1, second exemple réel).
...
| C4.3.2 — Journal de version            | 6       | 7 versions reconstituées avec fonctionnalités et anomalies corrigées, adossées au journal détaillé[dev/CHANGELOG.md](annexes/dev/CHANGELOG.md) ; les prochaines seront taguées automatiquement en git/Release GitHub (R1, §5)                                                                                                              |
